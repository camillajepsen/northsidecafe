...
if (!empty($register_date)) {
    ...
    if (empty($_POST['Contact number'])) {
        $Contact_number_err = 'Phone number is required';
    } else {
        $Contact_number = filter_input(INPUT_POST, 'Contact number', FILTER_SANITIZE_STRING);
    }
    ...
    if (empty($email_err) && empty($register_date_err) && empty($Contact_number_err) && empty($password_err)) {
...
$stmt->bind_param("ssssss", $email, $register_date, $Contact_number, $password, $membershipship_id, $staff_id);
...
<div class="wrapper">
    ...
    <form method="POST" action="success_create_member_memberships.php" class="ht-600 w-50">
        ...
        <div class="form-group">
            ...
            <input placeholder="+013 985 3921" type="text" name="Contact number" id="Contact number" required class="form-control <?php echo $Contact_number_err ? 'is-invalid' : ''; ?>" value="<?php echo $Contact_number; ?>"><br>
            <div class="invalid-feedback">
